...
import com.leo.stock.Bean.SinaSimpleStockBean;
...
public class StockBeanParser {
    ...
    private static void parse(SinaStockBean stockBean, String[] values) {
        if (values.length == 6) {
            ...
            stockBean.todayCurrentPrice = FloatUtil.handleFloat(values[1]);
            stockBean.priceChange = FloatUtil.handleFloat(values[2]);
            stockBean.priceChangePercent = FloatUtil.handleFloat(values[3]);
            stockBean.amount = Long.valueOf(values[4]) * 100;
            stockBean.turnover = Long.valueOf(values[5]) * 10000;
        } else if (values.length > 6) {
            stockBean.stockName = values[0];
            stockBean.todayOpenPrice = FloatUtil.handleFloat(values[1]);
            stockBean.lastClosePrice = FloatUtil.handleFloat(values[2]);
            stockBean.todayCurrentPrice = FloatUtil.handleFloat(values[3]);
            stockBean.highestPrice = FloatUtil.handleFloat(values[4]);
            stockBean.lowestPrice = FloatUtil.handleFloat(values[5]);
            stockBean.competitionBuyPrice = FloatUtil.handleFloat(values[6]);
            stockBean.competitionSellPrice = FloatUtil.handleFloat(values[7]);
            ...
            stockBean.buyPrice1 = FloatUtil.handleFloat(values[11]);
            stockBean.buyCount2 = Long.parseLong(values[12]);
            stockBean.buyPrice2 = FloatUtil.handleFloat(values[13]);
            stockBean.buyCount3 = Long.parseLong(values[14]);
            stockBean.buyPrice3 = FloatUtil.handleFloat(values[15]);
            stockBean.buyCount4 = Long.parseLong(values[16]);
            stockBean.buyPrice4 = FloatUtil.handleFloat(values[17]);
            stockBean.buyCount5 = Long.parseLong(values[18]);
            stockBean.buyPrice5 = FloatUtil.handleFloat(values[19]);
            stockBean.sellCount1 = Long.parseLong(values[20]);
            stockBean.sellPrice1 = FloatUtil.handleFloat(values[21]);
            stockBean.sellCount2 = Long.parseLong(values[22]);
            stockBean.sellPrice2 = FloatUtil.handleFloat(values[23]);
            stockBean.sellCount3 = Long.parseLong(values[24]);
            stockBean.sellPrice3 = FloatUtil.handleFloat(values[25]);
            stockBean.sellCount4 = Long.parseLong(values[26]);
            stockBean.sellPrice4 = FloatUtil.handleFloat(values[27]);
            stockBean.sellCount5 = Long.parseLong(values[28]);
            stockBean.sellPrice5 = FloatUtil.handleFloat(values[29]);
            stockBean.date = values[30];
            stockBean.time = values[31];

            stockBean.priceChange =
                    FloatUtil.handleFloat(stockBean.todayCurrentPrice - stockBean.lastClosePrice);
            stockBean.priceChangePercent =
                    FloatUtil.handleFloat(100 * stockBean.priceChange / stockBean.lastClosePrice);
        }
    }
}
